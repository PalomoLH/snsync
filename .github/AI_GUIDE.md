...
This repository uses a Node.js CLI tool (`node snsync`) to sync local files with a ServiceNow instance. The entry point is the root `snsync` file, which delegates to `_tool/sn-sync.js` via `_tool/bin/snsync.js`.
...
- `_tool/`: Tool source code (`sn-sync.js`, `flow-modifier.js`, `bin/snsync.js`).
